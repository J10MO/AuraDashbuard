...
// //                       value={formData.description_ar}
// //                       onChange={(e) => setFormData({ ...formData, description_ar: e.target.value })}
// //                       placeholder="خصومات رائعة على منتجات ��لصيف"
// //                       required
// //                       dir="rtl"
// //                       rows={3}
// //                     />
...
//       const adData = {
//         product_id: Number.parseInt(restFormData.product_id),
//         title: restFormData.title,
//         title_ar: restFormData.title_ar,
//         description: restFormData.description,
//         description_ar: restFormData.description_ar,
//         start_date: new Date(restFormData.start_date).toISOString(),
//         end_date: new Date(restFormData.end_date).toISOString(),
//         position: restFormData.position,
//         priority: Number.parseInt(restFormData.priority),
//         image: image,
//       }
...
export default function AdsPage() {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      const productsArray = response.products || response.data?.products || response.data || []
      console.log("[v0] Final products array:", productsArray)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!restFormData.product_id || !restFormData.title_ar || !restFormData.description_ar || !restFormData.start_date || !restFormData.end_date) {
                  ...
                  <div className="col-span-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        {products.map((product) => (
                          <SelectItem key={product.id} value={product.id.toString()}>
                            {product.name || "Unnamed Product"} - {product.name_ar || "منتج بدون اسم"}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
